...
def initialize_client():
    """
    Creates the AzureOpenAI client and registers it with Highflame.
    ...
    if not api_key:
        print("Error: HIGHFLAME_API_KEY is not set!")
        return None
    else:
        print("HIGHFLAME_API_KEY found.")
    ...
    # Initialize the Highflame client and register the Azure client
    config = Config(api_key=api_key)
    client = Highflame(config)
    client.register_azureopenai(azure_client)
...
def main():
    print("Azure OpenAI via Highflame Testing:")
