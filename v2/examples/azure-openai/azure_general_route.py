...
def init_azure_client_sync():
    ...
    try:
        ...
        if not llm_api_key or not api_key:
            raise Exception(
                "AZURE_OPENAI_API_KEY and HIGHFLAME_API_KEY must be set in "
                "your .env file."
            )
        javelin_headers = {"x-highflame-apikey": api_key}
        client = AzureOpenAI(
            api_key=llm_api_key,
            base_url=f"{os.getenv('HIGHFLAME_BASE_URL')}/v1/query/azure-openai",
...
def init_azure_embeddings_client_sync():
    ...
    try:
        ...
        client = AzureOpenAI(
            api_key=llm_api_key,
            base_url=("https://api.highflame.app/v1/query/azure_ada_embeddings"),
            default_headers=javelin_headers,
