...
javelin_base_url = os.getenv("HIGHFLAME_BASE_URL", "")

if not (openai_api_key and api_key and javelin_base_url):
    raise ValueError(
        "Missing OPENAI_API_KEY, HIGHFLAME_API_KEY, or HIGHFLAME_BASE_URL in .env"
    )
...
client = Highflame(
    Config(api_key=api_key, base_url=javelin_base_url)
)
# Adjust route name if needed
client.register_openai(async_openai_client, route_name="openai_univ")

# Let the Agents SDK use this Highflame-patched client globally
