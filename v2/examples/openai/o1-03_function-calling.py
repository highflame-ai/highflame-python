...
def init_client(openai_client, route_name="openai_univ"):
...
def init_client_azure(azure_client, route_name="azureopenai_univ"):
    api_key = os.getenv("HIGHFLAME_API_KEY")
    config = Config(api_key=api_key)
    client = Highflame(config)
...
def azure_function_call_non_stream():
    ...
    init_client_azure(azure_client)
...
def openai_regular_non_stream():
    ...
    client = Highflame(config)
    print("Successfully connected to Highflame Client for OpenAI")
