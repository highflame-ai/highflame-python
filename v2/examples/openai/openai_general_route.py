...
def init_sync_openai_client():
    """Initialize and return a synchronous OpenAI client with Highflame headers."""
    try:
...
def init_async_openai_client():
    """Initialize and return an asynchronous OpenAI client with Highflame headers."""
    try:
        openai_api_key = os.getenv("OPENAI_API_KEY")
        api_key = os.getenv("HIGHFLAME_API_KEY")
        javelin_headers = {"x-highflame-apikey": api_key}
...
def sync_openai_embeddings(_):
    ...
    try:
        ...
        embeddings_client = OpenAI(
            api_key=openai_api_key,
            base_url=("https://api.highflame.app/v1/query/openai_embeddings"),
            default_headers=javelin_headers,
        )
