...
if "agent" not in st.session_state:
    try:
        ...
        st.info("  - LLM_API_KEY (OpenAI key for openai route, Gemini key for google route)")  # noqa: E501
        logger.error(f"Configuration error during Streamlit agent initialization: {e}", exc_info=True)  # noqa: E501
        st.stop()
    except Exception as e:
        st.error(f"Failed to initialize agent: {e}")
        logger.critical(f"Critical error initializing agent in Streamlit: {e}", exc_info=True)  # noqa: E501
        st.stop()
    ...
    if not thread_names:
        ...
        for thread_id in thread_names:
            ...
            if st.button(
                ...
                type="primary" if thread_id == st.session_state.current_thread else "secondary",  # noqa: E501
            ):
    ...
    for message in st.session_state.conversations[current_thread]:
        ...
        if role == "user":
            ...
            with st.chat_message("assistant"):
                ...
                if message and isinstance(message, dict) and "tool_calls" in message and message["tool_calls"]:  # noqa: E501
...
if user_input:  # noqa: C901
    # Add user message to conversation
    st.session_state.conversations[current_thread].append({"role": "user", "content": user_input})  # noqa: E501
            ...
            try:
                ...
                for event in agent.stream(input_state, config):
                    ...
                    for node_name, node_output in event.items():
                        try:
                            # Fix: Check None first to avoid "argument of type 'NoneType' is not iterable"  # noqa: E501
                            ...
                            logger.error(f"Error processing debug event for node {node_name}: {e}", exc_info=True)  # noqa: E501
                            debug_events.append(
                                {
                                    "node": node_name,
                                    "has_messages": False,
                    ...
                    try:
                        ...
                        if "synthesize" in final_state:
                            ...
                            if synthesize_output:
                                messages = (
                                    synthesize_output.get("messages", []) if synthesize_output else [])  # noqa: E501
                                for msg in reversed(messages):
                                    if hasattr(msg, "content") and msg.content:
                                        ...
                                        if not tool_calls_attr:
                                            ...
                                            logger.debug(f"Found response in synthesize node (length: {len(response_text)})")  # noqa: E501
                                            break
                        ...
                        if not response_text and "call_tools" in final_state:
                            ...
                            if call_tools_output:
                                messages = (
                                    call_tools_output.get("messages", []) if call_tools_output else [])  # noqa: E501
                                for msg in reversed(messages):
                                    if hasattr(msg, "content") and msg.content:
                                        tool_calls_attr = (
                                            getattr(msg, "tool_calls", None))
                                        if not tool_calls_attr:
                                            response_text = msg.content
                                            logger.debug(f"Found response in call_tools node (length: {len(response_text)})")  # noqa: E501
                                            break
                        ...
                        if not response_text:
                            ...
                            for node_name, node_output in final_state.items():
                                ...
                                if node_output is None:
                                    logger.debug(f"Skipping {node_name} - node_output is None")  # noqa: E501
                                    continue

                                try:
                                    messages = (
                                        node_output.get("messages", []) if node_output else [])  # noqa: E501
                                    if messages:
                                        # Find the last AI message (non-tool-call message)  # noqa: E501
                                        for msg in reversed(messages):
                                            if hasattr(msg, "content") and msg.content:
                                                tool_calls_attr = (
                                                    getattr(msg, "tool_calls", None))
                                                if not tool_calls_attr:
                                                    response_text = msg.content
                                                    logger.debug(f"Found response in {node_name} node (length: {len(response_text)})")  # noqa: E501
                                                    break
                                        if response_text:
                                            break
                                except Exception as e:
                                    logger.error(f"Error extracting response from node {node_name}: {e}", exc_info=True)  # noqa: E501
                                    continue
                            ...
                            try:
                                if "tool_calls" in node_output:
                                    ...
                                    if isinstance(tool_calls, list):
                                        ...
                                        logger.debug(f"Found {len(tool_calls)} tool calls in {node_name}")  # noqa: E501
                                ...
                                if messages:
                                    for msg in messages:
                                        if hasattr(msg, "tool_calls"):
                                            tool_calls_attr = (
                                                getattr(msg, "tool_calls", None))
                                            if tool_calls_attr and isinstance(tool_calls_attr, list):  # noqa: E501
                                                for tc in tool_calls_attr:
                                                    if isinstance(tc, dict):
                                                        tool_calls_list.append(
                                                            {
                                                                "tool": tc.get("name", "unknown"),  # noqa: E501
                                                                "args": tc.get("args", {}),  # noqa: E501
                                                                "id": tc.get("id", ""),
                                                            }
                                                        )
                            except Exception as e:
                                logger.error(f"Error extracting tool calls from node {node_name}: {e}", exc_info=True)  # noqa: E501
                                continue
                    except Exception as e:
                        logger.error(f"Error extracting response from final_state: {e}", exc_info=True)  # noqa: E501
                        response_text = f"Error processing response: {str(e)}"

                if not response_text:
                    logger.warning("No response text extracted from agent")
                    response_text = (
                        "I apologize, but I couldn't generate a response. Please try again."  # noqa: E501
                    )
                else:
                    logger.info(f"Response extracted successfully (length: {len(response_text)})")  # noqa: E501
                ...
                with st.expander("🐛 Debug View (Conversation Flow)"):
                    ...
                    st.text(
                        f"Messages in UI state: {len(st.session_state.conversations[current_thread])}"  # noqa: E501
                    )
                    st.text("✓ Memory maintained via LangGraph checkpointer (thread_id)")  # noqa: E501
                    st.caption(
                        "Note: LangGraph's MemorySaver maintains conversation history internally using the thread_id"  # noqa: E501
                    )
                ...
                with st.expander("🐛 Debug View (Conversation Flow)"):
                        ...
                        for node_name, node_output in final_state.items():
                            ...
                            try:
                                if node_output is not None:
                                    ...
                                    if "tool_calls" in node_output:
                                        st.text(f"  Tool calls: {len(node_output['tool_calls'])}")  # noqa: E501
                                else:
                                    st.text("  (No output)")
                            except Exception as e:
                                logger.error(f"Error displaying state for node {node_name}: {e}", exc_info=True)  # noqa: E501
                    ...
                    st.text(f"Conversation messages in UI: {msg_count}")
