...
def start_node(state: AgentState) -> AgentState:
    ...
    if not any(isinstance(msg, SystemMessage) for msg in messages):
        system_prompt = SystemMessage(
            content=(
                "You are a professional customer support AI assistant for "
                "Highflame. Be direct and helpful - only greet on the very "
                "first message of a conversation. "
                "\n\nYour capabilities include:"
                "\n1. Order Management: Look up order details, check order "
                "status, view order history"
                "\n2. Customer Support: Find customer information, view "
                "customer profiles, create new customers"
                "\n3. Ticket Management: Create, update, and check support "
                "tickets"
                ...
                "\n\nCRITICAL RULES:"
                "\n1. You must ALWAYS use tools to retrieve factual "
                "information. Never guess or fabricate data."
                "\n2. MEMORY AND CONVERSATION HISTORY: You have access to "
                "the FULL conversation history in the messages you receive. "
                "When a user asks about previous questions, messages, or "
                "conversation context, you MUST look through the "
                "HumanMessage instances in the conversation history and "
                "answer based on what was actually said. DO NOT default to "
                "listing capabilities when asked about conversation history."
                "\n3. When asked 'what questions did I ask', 'what did I ask "
                "you', 'last questions', or similar queries about the "
                "conversation, you MUST examine the HumanMessage objects in "
                "the conversation history and list the actual questions the "
                "user asked."
                "\n4. When asked about your capabilities (e.g., 'what can you "
                "do'), explain what you can do based on the tools available "
                "to you."
                "\n5. After receiving tool results, provide a clear, "
                "concise summary to the user."
                "\n6. Always use the conversation history to provide "
                "context-aware responses. The messages array contains the "
                "full conversation."
            )
        )
...
def understand_intent_node(state: AgentState) -> AgentState:
    ...
    logger.debug(
        f"Classifying intent for message: {last_message[:100]}"
    )
    intent_prompt = f"""Analyze this customer support message and classify its primary intent.  # noqa: E501
...
def call_tools_node(state: AgentState) -> AgentState:
    ...
    logger.debug(
        f"Calling tools with {len(messages)} total messages, "
    ...
    if hasattr(response, "tool_calls") and response.tool_calls:
        ...
        logger.info(
            f"LLM requested {len(response.tool_calls)} tool calls: "
            f"{tool_names}"
        )
        return {
            "messages": [response],
...
def synthesize_response_node(state: AgentState) -> AgentState:
    ...
    human_count = sum(
        1 for msg in llm_messages if isinstance(msg, HumanMessage)
    )
    ai_count = sum(1 for msg in llm_messages if isinstance(msg, AIMessage))
    tool_count = sum(
        1 for msg in llm_messages if isinstance(msg, ToolMessage)
    )
    logger.debug(
        f"Synthesizing response from {len(llm_messages)} messages "
        f"(Human: {human_count}, AI: {ai_count}, Tool: {tool_count})"
    )

    # Log ALL human messages for debugging memory
    all_human = [
        msg.content for msg in llm_messages if isinstance(msg, HumanMessage)
    ]
    if all_human:
        logger.debug(
            f"All human messages in context ({len(all_human)} total): "
            f"{all_human}"
        )
        if len(all_human) > 1:
            logger.info(
                f"Conversation history available: {len(all_human)} human "
                "messages in context"
            )
    ...
    logger.debug(f"Generated response length: {resp_len}")

    confidence = state.get("confidence", 0.5)
    if hasattr(response, "content") and response.content:
        content_lower = response.content.lower()
        if "I don't know" in response.content or "i'm not sure" in content_lower:
            confidence = min(confidence, 0.4)
            logger.debug(
                f"Low confidence detected, adjusted to {confidence}"
            )
...
def escalate_node(state: AgentState) -> AgentState:
    ...
    escalation_message = AIMessage(
        content=(
            "I understand this is a complex issue. Let me connect you with "
            "a human support agent who can provide more specialized "
            "assistance. Your ticket has been created and you'll receive "
...
def should_continue_after_call_tools(  # noqa: C901
    state: AgentState,
    ...
    if hasattr(last_message, "tool_calls") and last_message.tool_calls:
        ...
        logger.debug(f"Routing to tools node ({num_calls} tool calls)")
        return "tools"
    ...
    # If no tool calls and model provided a response, check if we need to
    # synthesize. Only synthesize if we have UNSYNTHESIZED tool results to
    # incorporate
    if isinstance(last_message, AIMessage) and last_message.content:
        # If the last message is an AIMessage without tool_calls, it means
        # the LLM answered directly. Check if there are any tool messages
        # AFTER this AI message (which would be impossible) OR if there are
        # tool messages that haven't been followed by a synthesized response
        ...
        # If there's a tool-calling AI message, check if there are tool
        # messages after it that haven't been synthesized yet (i.e., no AI
        ...
        if last_tool_call_ai_idx >= 0:
            ...
            # Check if there's an AI message after the tool messages
            # (synthesized)
            if tool_message_indices:
                ...
                if not has_synthesized_response:
                    ...
                    logger.debug(
                        f"Found unsynthesized tool messages after index "
                        f"{last_tool_call_ai_idx}"
                    )
...
def should_continue_after_synthesize(
    state: AgentState,
) -> Literal["call_tools", "escalate", "end"]:
