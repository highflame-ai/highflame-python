...
async def root():
    ...
    return {
        ...
        "endpoints": {
            "/chat": (
                "POST - Chat with the agent (maintains conversation state)"
            ),
...
async def chat(request: ChatRequest):  # noqa: C901
    """
    ...
    logger.info(
        f"Chat request received - thread_id: {thread_id}, "
    ...
    try:
            ...
            if not response_text:
                for node_name, node_output in final_state.items():
                    ...
                    if messages:
                        for msg in reversed(messages):
                            if hasattr(msg, "content") and msg.content:
                                if not (hasattr(msg, "tool_calls") and
                                        msg.tool_calls):
                                    ...
                                    if msg.content not in greetings:
                                        response_text = msg.content
                                        break
                        if response_text:
                            break
        ...
        if not response_text:
            logger.warning(
                f"No response text extracted for thread: {thread_id}"
            )
            response_text = (
            ...
            logger.info(
                f"Response generated - length: {len(response_text)}, "
...
async def generate(request: GenerateRequest):  # noqa: C901
    """
    ...
    msg_len = len(request.message)
    logger.info(
        f"Generate request received - thread_id: {thread_id}, "
        f"message length: {msg_len}"
    )
    ...
    try:
        ...
        logger.debug(
            f"Agent completed with {event_count} events, "
            f"{len(all_states)} states collected"
        )
        ...
        # Priority 1: If tools were called, use synthesize node from final
        # state
        ...
        # Priority 2: If no tools called, use call_tools node response
        # (check all states)
        if not response_text:
                    ...
                    for msg in reversed(messages):
                        if (isinstance(msg, AIMessage) and
                            ...
                            if not (hasattr(msg, "tool_calls") and
                                ...
                                # Always use call_tools response if it exists
                                # and is not empty
                                if content:
                ...
                for msg in reversed(messages):
                    if (isinstance(msg, AIMessage) and
                        ...
                        if not (hasattr(msg, "tool_calls") and
                            ...
                            if content and content not in greetings:
                                response_text = content
                                break
        ...
        import traceback
        error_detail = (
            f"Error processing generate request: {str(e)}\n"
